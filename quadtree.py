...
# base classes for more sophistivated implementations.
...
class Node(): 
    ...
    # a point a within the bounding box of the node.
    def contains(self, x, z):
        x0,z0,x1,z1 = self.rect
        if x >= x0 and z <= x1 and z >= z0 and z <= z1:
            return True
        return False
    #------------------------------------------------------
...
class QuadTree():
    ...
    #--------------------------------------------------------
    # Sets the children of 'node' to None if they do not have any 
    # LEAF nodes.
    def prune(self, node):
